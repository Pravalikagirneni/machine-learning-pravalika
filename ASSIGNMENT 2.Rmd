...
```{r}
library(class)  # knn is present in class package
...
## import a universal bank dataset 
...
#Eliminating variables [id & zip code] from the data set
```{r}
...
dummy_Education_1 <- as.data.frame(dummy.code(df$Education))
names(dummy_Education_1) <- c("Education_1", "Education_2","Education_3") #renaming the dummy variable
df_without_education <- subset(df, select=-c(Education))                  #eliminating education variable

UBank_data <- cbind(df_without_education, dummy_Education_1)              #main dataset
```

###Data partition 

# Spliting the data into Two Groups Traning(60%) and Validation(40%)
```{r}

set.seed(1234)
Train_Index     = createDataPartition(UBank_data$Age, p= 0.6 , list=FALSE) #Traning group data 60%
...
#Data Normalization(data has to be normalized before the process)
```{r}
...
# Using KNN CLASSIFICATION METHOD, using k = 1
```{r}
set.seed(1234)
...
#3: confusion matrix for the best k value =1
table(prediction,actual)  
...
mean(prediction_Q5==actual)  #accuracy of the best k=1
```
THE K VALUE WHICH WE GIVE IN TEST TRAIN VALID WILL BE DIFFERENT,AND ACCURACY WILL ALSO BE DIFFERENT,SO DOES CLASSIFICATION